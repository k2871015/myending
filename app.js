...
// Helper to mask codes safely (e.g. 4-digit code 0416 -> **16)
  ...
  if (startBtn) {
    startBtn.addEventListener('click', () => {
      ...
      if (!phoneVal || phoneVal.length !== 4) {
        showToast('네 자리 숫자 잠금번호를 올바르게 입력해 주세요.');
    ...
    <div class="screen-state" style="animation: fadeIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards;">
      <div class="intro-content">
        ...
        <div class="owner-badge" style="font-size: 0.78rem; color: var(--color-gold); background: rgba(226, 201, 116, 0.08); padding: 0.35rem 0.8rem; border-radius: 12px; border: 1px dashed rgba(226, 201, 116, 0.25); margin-top: -0.5rem; margin-bottom: 0.5rem; letter-spacing: 0.05em; display: inline-block;">
          🔐 소유자: ${ownerName} (잠금코드: ${maskPhoneNumber(ownerPhone)})
        </div>
        <p class="desc">
...
async function activateRelic() {
  ...
  const statusTexts = [
    ...
    `잠금코드 (${maskPhoneNumber(ownerPhone)}) 검증 및 화면 잠금 해제 중...`,
    `기억의 파편 복원 중...`,
    `마지막 마음의 조각 직조 중...`
  ];
  ...
  screenContainer.innerHTML = `
    <div class="screen-state" id="setup-state" style="animation: fadeIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards;">
      <div class="intro-content">
        ...
        <h3>흔적 동조</h3>
        <p class="desc">
          떠난 이의 이름과 기억의 비밀번호를 입력하여, 기기 속에 남겨진 기억과 온기를 소환합니다.
        </p>
        ...
        <div class="setting-panel" style="border: none; margin: 1rem 0 0 0; padding: 0; width: 100%;">
          ...
          <div class="form-group" style="margin-top: 1rem;">
            <div class="input-label">기억의 비밀번호 (네 자리 숫자)</div>
            <input type="text" maxlength="4" pattern="[0-9]{4}" id="owner-phone-input" class="text-input" placeholder="예: 0416 (기억의 날짜 등)" autocomplete="off" required>
          </div>
          <div class="form-group" style="margin-top: 1rem;">
...
The owner of this phone is named "${ownerName}" (lock passcode: "${ownerPhone}").
...
Make sure to refer to the deceased owner by their name "${ownerName}" throughout the story (instead of generic "그" or "그 사람"), and occasionally weave in details related to their lock passcode "${ownerPhone}" if appropriate (e.g. how they locked their secrets under this code, or how the passcode was their most beautiful day).
