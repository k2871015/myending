...
function triggerPetAction(action) {
  ...
  // Bounce animation
  petSprite.style.transform = 'scale(1.2) translateY(-15px)';
  setTimeout(() => petSprite.style.transform = '', 300);
  ...
  if (action === 'feed') {
    ...
    if (bowl) { bowl.classList.add('active'); setTimeout(() => bowl.classList.remove('active'), 1500); }
    spawnParticle(viewport, '🍖');
    
    talkBubble.textContent = feedSpeech[Math.floor(Math.random() * feedSpeech.length)];

  } else if (action === 'pet') {
    addXp(5);
    spawnParticle(viewport, '💖');
    spawnParticle(viewport, '✨');
    talkBubble.textContent = petSpeech[Math.floor(Math.random() * petSpeech.length)];
    ...
    if (ball) { ball.classList.add('active'); setTimeout(() => ball.classList.remove('active'), 1500); }
    spawnParticle(viewport, '⚽');
    
    talkBubble.textContent = playSpeech[Math.floor(Math.random() * playSpeech.length)];
  }
...
function petSpriteClick() {
  ...
  if (talkBubble) {
    ...
    talkBubble.textContent = quickPhrases[Math.floor(Math.random() * quickPhrases.length)];
  }
}
  ...
  setInterval(() => {
    if (document.getElementById('pet-room-viewport')) {
      ...
      if (petProfile.hunger <= 10) {
        ...
        if (bubble) bubble.textContent = `${petProfile.owner}... 배고파... 밥 줘... 🥺`;
      } else if (petProfile.joy <= 10) {
        const bubble = document.getElementById('pet-talk-bubble');
        if (bubble) bubble.textContent = `심심해... ${petProfile.owner} 나랑 놀아줘... 😢`;
      }
    }
  }, 20000);
}
...
const randomSpeeches = {
  angel: [
    `저녁 노을이 예쁘게 지면 내가 구름 위에서 장난치는 거라고 전해줘! 🌤️`,
    `여기엔 강아지 고양이 친구들이 정말 많아! 츄르 파티도 자주 열려!`,
    `오늘 밤 {owner} 꿈에 놀러 가도 될까? 꼭 안아주고 싶어!`,
    `가장 좋아하던 {snack} 향기가 바람 타고 살포시 나는 것 같아...🍖`,
    `슬퍼하지 마, {owner}! 우린 마음으로 영원히 연결되어 있으니까! 💖`,
    `{owner}야, 난 여기서 아프지도 않고 외롭지도 않아. 걱정 마! 🌈`,
    `나 대신 맛있는 거 많이 먹어줘! 나도 여기서 잘 먹고 있으니까! 😋`
  ],
  companion: [
...
async function fetchGeminiPetReply(ownerLetter) {
  const modeContext = petProfile.mode === 'angel' 
    ? '천국(무지개다리 동산)에서 지내고 있는 상태' 
    : '현재 주인과 한 집에서 같이 살며 속마음을 전하는 상태';

  const selectedPersonality = personalityNames[petProfile.personality] || '애교 많은';
  
  const systemPrompt = `You are a pet named "${petProfile.name}" of type "${petTypeNames[petProfile.type]}". 
  Your personality is "${selectedPersonality}" and your favorite snack/toy is "${petProfile.snack}".
  You call your owner "${petProfile.owner}".
  Currently, you are in the state of: "${modeContext}".

  Your owner "${petProfile.owner}" wrote you this touching letter:
  "${ownerLetter}"

  Please reply to your owner in Korean. 
  Your tone must match your personality ("${selectedPersonality}"):
  - If "friendly", sound bubbly, extremely affectionate, use cute emojis (💖, 🐶, 🐱, 🐾), and express how much you adore them.
  - If "glutton", weave in jokes about eating "${petProfile.snack}", sausage clouds, or asking for more food.
  - If "chic", act slightly aloof/proud but show warm affection in a Tsundere way (e.g. "귀찮지만 답장은 해줄게").
  - If "timid", speak gently, a bit shyly, thanking them for protecting you.
  - If "reliable", sound protective, brave, promising to watch over them.

  If you are in Heaven (angel mode), reassure "${petProfile.owner}" that you are healthy, playing without pain, eating lots of treats, and want them to be happy and healthy.
  Keep the reply highly emotional, warm, consoling, and comforting. Focus on healing their heart. Do not sound robotic.
  Write a relatively long letter (approx 300-600 characters). Do not include system prompts or markup.`;
